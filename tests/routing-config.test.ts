...
test("routing config accepts OpenAI-only credentials without eager Alibaba config", () => {
  ...
  assert.equal(config.gemini, undefined);
  assert.equal(config.alibaba, undefined);
});

test("routing config supports Gemini aliases and explicit model overrides", () => {
  ...
  assert.deepEqual(config.gemini, {
    ...
    analysisModel: "gemini-custom",
    imageModel: "gemini-image-custom",
  });
});
